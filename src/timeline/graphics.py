from timeline.svg_primitives import svg_rect
...
class viewport:
    def __init__(self, x, y, width, height, min_date: str, max_date: str,
                 font_family="Arial", font_size=14, padding=(5, 5),
                 spacing=(5, 5), color="transparent"):
        self.x = x
        self.y = y
        self.width = width
        self._height = height
        ...
        ttf_font = font_manager.FontProperties(
            family=font_family, weight='normal')
    ...
    def __str__(self):
        return f'viewport: x={round(self.x)}, y={round(self.y)}, ' + \
            f'height={round(self.height)}, width={round(self.width)}'

    def add_viewport(self, x_offset=0, height=0, spacing=(5, 5),
                     padding=(5, 5), color="transparent"):
        next_y = self.y + self.height
        temp = viewport(
            self.x + self.spacing[0] + x_offset, next_y,
            self.width - x_offset - 2 * self.spacing[0], height,
            self.min_date.strftime("%d-%m-%Y"),
            self.max_date.strftime("%d-%m-%Y"), self.font_family,
            self.font_size, padding, spacing, color=color)
    ...
    @property
    def height(self):
        out = self._height + sum([i.height + self.spacing[1] for i in
                                  self.viewports])
        return out
    ...
    def render_background(self, debug=False):
        ...
        out += svg_rect(
            self.x, self.y, self.width, self.height, fill_color="#f0f0f0",
            lwd=0)
        for i in self.viewports:
    ...
    def render_svg_header(self, width=None):
        ...
        svg_out = f'<svg width="{width}" height="{
            self.height + self.y + self.spacing[1] + 10}" viewbox="0 0 {
                self.width + self.spacing[0] * 2} {
                    self.height/self.width
                    }" xmlns="http://www.w3.org/2000/svg">\n'
        svg_out += f'<style>text {{font-family: {
            self.font_family}; font-size: {self.font_size}px ;}}</style>\n'
        return svg_out
